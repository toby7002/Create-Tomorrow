...
REIEvents.groupEntries(function (event) {
    event.groupItems("kubejs:rei_groups/rechiseledcreate", "Rechiseled: Create Blocks", [Ingredient.of("@rechiseledcreate").getItemIds()]);
    event.groupItems("kubejs:rei_groups/rechiseled", "Rechiseled Blocks", [
        Ingredient.of("@rechiseled").getItemIds().filter(function (item) { return item !== "rechiseled:chisel"; }),
    ]);
    event.groupItems("kubejs:rei_groups/chipped", "Chipped Blocks", [
        Ingredient.of("@chipped").getItemIds().filter(function (item) {
            return item !== "chipped:mason_table" &&
...
function CreateTheFactoryMustGrowFluids() {
    function Fluids() {
        return ["molten_slag", "molten_steel"].map(function (fluid) { return "tfmg:".concat(fluid); });
    }
    return Fluids();
}
REIEvents.hide("item", function (event) {
    getMekanism()
        .concat(getCreateNewAge(), getCreateAddition(), getTwilightForest(), CreateTheFactoryMustGrowItems(), AdAstra())
